...
exports.sourceNodes = ({actions}, configOptions) => {
    ...
    const browsePaginated = async (browseResource, options = {}) => {
        const pageSize = 100;
        const normalizedOptions = {...options};
        delete normalizedOptions.limit;
        delete normalizedOptions.page;

        let page = 1;
        let combinedRecords = [];

        while (true) {
            const records = await browseResource.browse({
                ...normalizedOptions,
                limit: pageSize,
                page
            });
            ...
            const pagination = _.get(records, 'meta.pagination');
            if (pagination) {
    ...
    const fetchPosts = browsePaginated(api.posts, postAndPageFetchOptions)
        .then((posts) => {
            posts = transformCodeinjection(posts);
            posts.forEach(post => createNode(PostNode(post)));
        }).catch(ignoreNotFoundElseRethrow);

    const fetchPages = browsePaginated(api.pages, postAndPageFetchOptions)
        .then((pages) => {
    ...
    const fetchTags = browsePaginated(api.tags, tagAndAuthorFetchOptions)
        .then((tags) => {
            tags.forEach((tag) => {
    ...
    const fetchAuthors = browsePaginated(api.authors, tagAndAuthorFetchOptions)
        .then((authors) => {
